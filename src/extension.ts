					...
					setTimeout(
						() =>
							st.serial.flush(() => {
								...
								const msg = index;
								serialEmitter.emit('updatePercentage', msg);
							}),
						100 + index * 10
					);
				});
				...
				setTimeout(
					() =>
						st.serial.flush(() => {
							...
							serialEmitter.emit('downloadFinished', msg);
						}),
...
function updateProgressBar(
	...
	return new Promise<void>(resolve => {
		...
		var messageUpdate = 'Starting download';
		var incrementUpdate = 0;
		var timerUpdate = 500;
		...
		let childProcess = serialEmitter.on('downloadFinished', msg => {
			resolve();
			clearInterval(interval);
		});

		childProcess.on('updatePercentage', msg => {
			const p = percentageParser(splitDataLen, msg);
	...
	vscode.window.withProgress(
		...
		async (progress, token) => {
			token.onCancellationRequested(() => {
				console.log('User canceled script download');
			});
